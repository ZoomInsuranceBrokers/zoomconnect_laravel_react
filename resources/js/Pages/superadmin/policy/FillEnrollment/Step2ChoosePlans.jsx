...
export default function Step2ChoosePlans({
    ...
    function getBaseSumInsured(cfg) {
        ...
        const value = cfg.base_sum_insured;

        if (type === 'fixed' || type === 'grade_wise' || type === 'gradewise') {
            if (type === 'grade_wise' || type === 'gradewise') {
                const grade = (employee?.grade || '').toString();
                const gradeOptions = cfg.grade_wise_sum_insured || [];

                if (Array.isArray(gradeOptions) && grade) {
                    const found = gradeOptions.find((g) => String(g.grade_name) === String(grade));
    ...
    const handleNext = () => {
        ...
        if (typeof updateFormData === "function") {
            ...
            const selectedPlanObj = plansWithBase.find((p) => String(p.id) === String(selection.selectedPlanId));
            // Prepare selection with plan name instead of id
            const selectionWithName = {
                ...selection,
                selectedPlanName: selectedPlanObj ? selectedPlanObj.plan_name : null,
                selectedPlanId: undefined // Remove id, only keep name
            };
            updateFormData({
                ...formData,
    ...
    const PlanCard = ({ plan }) => {
        ...
        const sumInsured = plan.sum_insured !== undefined && plan.sum_insured !== null && plan.sum_insured !== '' && !isNaN(plan.sum_insured) ? Number(plan.sum_insured) : null;
            ...
            return (
                <label className={`p-3 border-2 border-dashed rounded-md cursor-pointer flex items-start gap-3 bg-white ${selected ? "border-[#934790] shadow-md" : "border-blue-300 hover:bg-blue-50"}`}>
                    ...
                    <div className="flex-1">
                        ...
                        <p className="text-xs text-blue-700 mt-1">Sum Insured: {sumInsured !== null ? formatCurrencyLocal(sumInsured) : 'N/A'}</p>
        ...
        return (
            <label className={`p-3 border rounded-md cursor-pointer flex items-start gap-3 ${selected ? "border-[#934790] shadow-md bg-white" : "border-gray-200 hover:bg-gray-50"}`}>
                ...
                <div className="flex-1">
                    ...
                    <p className="text-xs text-gray-600 mt-1">Sum Insured: {sumInsured !== null ? formatCurrencyLocal(sumInsured) : 'N/A'}</p>
                    {shouldProrate && (
