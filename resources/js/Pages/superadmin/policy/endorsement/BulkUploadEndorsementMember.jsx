...
export default function BulkUploadEndorsementMember({ endorsement, action_type }) {
    ...
    const title = isAddAction ? "Bulk Add Endorsement Members" : "Bulk Remove Endorsement Members";
    ...
    const handleUploadAndValidate = async () => {
        ...
        try {
            const response = await fetch(
                route("superadmin.policy.endorsements.upload-bulk-csv", endorsement.id),
                {
                    method: "POST",
                    body: formData,
                    headers: {
    ...
    const handleConfirmAndProcess = () => {
        ...
        setProcessing(true);
        router.post(
            route("superadmin.policy.endorsements.process-bulk-action", endorsement.id),
            {
    ...
    const downloadSample = () => {
        ...
        window.location.href = route("superadmin.policy.endorsements.download-sample-csv", [endorsement.id, type]);
    };

    return (
        <SuperAdminLayout>
            <Head title={`${title} - Endorsement #${endorsement?.endorsement_no || endorsement?.id}`} />
            <div className="p-4 max-w-5xl mx-auto">
                ...
                {flash?.message && (
                    <div className={`mb-4 p-3 rounded-md text-sm ${flash.messageType === "success" ? "bg-green-100 border border-green-400 text-green-700" : "bg-red-100 border border-red-400 text-red-700"}`}>{flash.message}</div>
                )}
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-4">
                    ...
                    <button onClick={downloadSample} className="mt-3 bg-blue-600 hover:bg-blue-700 text-white px-3 py-1.5 rounded-md text-xs font-medium">Download Sample CSV</button>
                </div>
                {!preview && (
                    <div className="bg-white rounded-lg shadow p-6">
                        ...
                        <div className="mt-6 flex justify-end gap-3">
                            <Link href={route("superadmin.policy.endorsements.show", endorsement.id)} className="px-4 py-2 border border-gray-300 rounded-md text-sm text-gray-700 hover:bg-gray-50">Cancel</Link>
                            <button onClick={handleUploadAndValidate} disabled={!file || uploading} className={`px-4 py-2 rounded-md text-sm text-white ${!file || uploading ? "bg-gray-400 cursor-not-allowed" : "bg-[#934790] hover:bg-[#7e3d7c]"}`}>{uploading ? "Validating..." : "Upload & Validate"}</button>
                        </div>
                    </div>
                )}
                {preview && (
                    <div className="bg-white rounded-lg shadow p-6 mt-6">
                                ...
                                <div className="bg-red-50 border border-red-200 rounded-lg p-4 max-h-64 overflow-y-auto">
                                    {preview.invalid_rows.map((row, index) => (
                                        <div key={index} className="mb-2 pb-2 border-b border-red-200 last:border-0">
                                            <p className="text-xs text-red-800 font-medium">Member: {row["Member Code"]} - {row["Email"]}</p>
                        ...
                        <div className="flex justify-end gap-3 mt-6">
                            <button onClick={() => { setPreview(null); setFile(null); }} className="px-4 py-2 border border-gray-300 rounded-md text-sm text-gray-700 hover:bg-gray-50">Upload Different File</button>
                            <button onClick={handleConfirmAndProcess} disabled={processing || preview.valid === 0} className={`px-4 py-2 rounded-md text-sm text-white ${processing || preview.valid === 0 ? "bg-gray-400 cursor-not-allowed" : "bg-green-600 hover:bg-green-700"}`}>{processing ? "Processing..." : `Confirm & Process ${preview.valid} Records`}</button>
                        </div>
                    </div>
                )}
            </div>
