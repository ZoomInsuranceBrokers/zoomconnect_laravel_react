...
export default function Index({ insurances }) {
    ...
    const filteredInsurances = insurances.filter((insurance) => {
        const matchesSearch =
            insurance.insurance_company_name.toLowerCase().includes(search.toLowerCase()) ||
            (insurance.city_name &&
                insurance.city_name.toLowerCase().includes(search.toLowerCase())) ||
            (insurance.state_name &&
                insurance.state_name.toLowerCase().includes(search.toLowerCase()));

        const matchesStatus =
            statusFilter === "all" ||
            (statusFilter === "active" && insurance.status === 1) ||
            (statusFilter === "inactive" && insurance.status === 0);
    ...
    useEffect(() => {
        const pages = Math.ceil(filteredInsurances.length / itemsPerPage);
        setTotalPages(pages);
        if (currentPage > pages && pages > 0) {
            setCurrentPage(pages);
        }
    }, [filteredInsurances.length, itemsPerPage, currentPage]);
    ...
    const handleToggleStatus = (id, currentStatus) => {
        if (confirm(`Are you sure you want to ${currentStatus === 1 ? 'deactivate' : 'activate'} this insurance company?`)) {
            router.put(route('superadmin.policy.insurance.toggle-status', id), {}, {
                preserveScroll: true,
                onSuccess: () => {
    ...
    const handleEdit = (id) => {
        router.get(route('superadmin.policy.insurance.edit', id));
    };

    const handleDelete = (id) => {
        if (confirm('Are you sure you want to delete this insurance company?')) {
            router.delete(route('superadmin.policy.insurance.destroy', id), {
                preserveScroll: true,
                onSuccess: () => {
                    setOpenActionsDropdown(null);
                },
            ...
            >
                <div className="flex justify-between items-center mb-4">
                    <div className="flex items-center gap-3">
                        <Link
                            href={route("superadmin.policy.insurance.create")}
                                ...
                                >
                                                ...
                                                <button
                                                    ...
                                                    onClick={(e) => {
                                                        ...
                                                        setOpenActionsDropdown(
                                                            openActionsDropdown === insurance.id
                                                                ? null
                                                                : insurance.id
                                                        );
                                                    }}
                                                    ...
                                                    >
                                                        <div className="py-1">
                                                            ...
                                                            <button
                                                                onClick={(e) => {
                                                                    e.stopPropagation();
                                                                    handleToggleStatus(insurance.id, insurance.status);
                                                                }}
                                                                className="block w-full text-left px-4 py-2 text-xs text-gray-700 hover:bg-gray-100"
                                                            >
                                                                <svg
                                                                ...
                                                                {insurance.status === 1 ? 'Deactivate' : 'Activate'}
                                                            </button>
                                                            <button
                                                                onClick={(e) => {
                                                                    e.stopPropagation();
                                    ...
                                    <td className="px-3 py-2 whitespace-nowrap">
                                        <div className="flex items-center gap-2">
                                            {insurance.insurance_comp_icon_url && (
                                                <img
                                                    src={`/${insurance.insurance_comp_icon_url}`}
                                                    alt={insurance.insurance_company_name}
