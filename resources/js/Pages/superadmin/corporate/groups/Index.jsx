...
export default function Index({ groups }) {
    ...
    const [modalData, setModalData] = useState({ group_name: '', remark: '', is_active: true });
    ...
    const filteredGroups = groups.filter(group => {
        const matchesSearch =
            ...
            (group.remark && group.remark.toLowerCase().includes(search.toLowerCase()));
        const matchesFilter =
            filter === 'all' ||
    ...
    // Update total pages when filtered groups change
    useEffect(() => {
        setTotalPages(Math.max(1, Math.ceil(filteredGroups.length / itemsPerPage)));
        // Reset to first page when filters change
        setCurrentPage(1);
    }, [filteredGroups.length, itemsPerPage, search, filter]);
    ...
    const openCreateModal = () => {
        setModalData({ group_name: '', remark: '', is_active: true });
        setIsEditMode(false);
        setIsModalOpen(true);
    };
    ...
    const openEditModal = (group) => {
        setModalData({ id: group.id, group_name: group.group_name, remark: group.remark });
        setIsEditMode(true);
        setIsModalOpen(true);
    };
    ...
    const handleModalSubmit = async (e) => {
        ...
        try {
            if (isEditMode) {
                // Update group
                await router.put(route('corporate.groups.update', modalData.id), modalData);
                setMessage({ type: 'success', text: 'Corporate Group updated successfully' });
            } else {
                // Create new group
                await router.post(route('corporate.groups.store'), modalData);
                setMessage({ type: 'success', text: 'Corporate Group created successfully' });
            }
        } catch (error) {
            setMessage({ type: 'error', text: 'An error occurred while saving the group' });
        }
    ...
    const toggleStatus = async (group) => {
        try {
            await router.put(route('corporate.groups.update', group.id), {
                ...group,
                is_active: !group.is_active,
            });
            setMessage({ type: 'success', text: `Group ${group.is_active ? 'deactivated' : 'activated'} successfully` });
        } catch (error) {
            setMessage({ type: 'error', text: 'An error occurred while updating the group status' });
        }
    };

    return (
        <SuperAdminLayout>
            <Head title="Corporate Groups" />
            <div className={`p-4 h-full overflow-y-auto ${isModalOpen ? 'blur-sm' : ''}`}>
                {/* Header */}
                ...
                <div className={`rounded-lg p-3 mb-4 text-[10px] md:text-xs flex items-center ${darkMode ? 'bg-blue-900 text-blue-200' : 'bg-blue-100 text-blue-800'}`}>
                    ...
                    <span>Groups can be assigned to organize corporate clients effectively.</span>
                </div>
                    ...
                    <table className="w-full min-w-[600px] border-collapse">
                        ...
                        <tbody className={`divide-y ${darkMode ? 'divide-gray-700' : 'divide-gray-200'}`}>
                            {currentItems.map((group) => (
                                <tr key={group.id} className={darkMode ? 'hover:bg-gray-700 even:bg-gray-800/50' : 'hover:bg-gray-50 even:bg-gray-50/50'}>
                                    ...
                                    <td className="px-6 py-3 whitespace-nowrap">
                                        <label className="inline-flex items-center cursor-pointer">
                                            <input
                                                ...
                                                onChange={() => toggleStatus(group)}
                                                className="sr-only"
                                            />
                                            <div
                                                className={`w-10 h-5 rounded-full transition-colors duration-200 ${group.is_active ? 'bg-[#934790]' : 'bg-gray-300'
                                                    } flex items-center`}
                                            >
                                                <div
                                                    className={`w-4 h-4 rounded-full bg-white shadow-md transform transition-transform duration-200 ${group.is_active ? 'translate-x-5' : 'translate-x-0'
                                                        }`}
                                                ></div>
                                            </div>
                                        </label>
                ...
                <div className="sm:hidden">
                    {currentItems.length === 0 ? (
                        <div className={`text-center py-6 text-xs ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>No groups found.</div>
                    ) : (
                                ...
                                >
                                    ...
                                    <div className="flex items-center gap-2">
                                        <label className="inline-flex items-center cursor-pointer">
                                            ...
                                            />
                                            <div
                                                className={`w-7 h-4 rounded-full transition-colors duration-200 ${group.is_active ? 'bg-[#934790]' : 'bg-gray-300'} flex items-center`}
                                            >
                                                <div
                                                    className={`w-3 h-3 rounded-full bg-white shadow-md transform transition-transform duration-200 ${group.is_active ? 'translate-x-4' : 'translate-x-0'}`}
                                                ></div>
                                            </div>
                                        </label>
                                    </div>
                                    ...
                                    <div className="flex-1 flex items-center gap-2">
                                        <span className={`text-[12px] font-semibold ${darkMode ? 'text-white' : 'text-gray-900'}`}>{group.group_name}</span>
                                    </div>
                                    {/* Actions */}
                        ...
                        <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                            <div>
                                <p className="text-[11px] md:text-xs text-gray-500 dark:text-gray-400">
                                    Showing <span className="font-medium">{Math.min((currentPage - 1) * itemsPerPage + 1, filteredGroups.length)}</span> to{' '}
                                    <span className="font-medium">{Math.min(currentPage * itemsPerPage, filteredGroups.length)}</span> of{' '}
                                    <span className="font-medium">{filteredGroups.length}</span> results
                                </p>
                            </div>
                            <div>
                        ...
                        <div className={`rounded-2xl shadow-xl p-6 max-w-md w-[90%] sm:w-[500px] ${darkMode ? 'bg-gray-800 text-white' : 'bg-white text-black'}`}>
                            <div>
                                <div className="flex justify-between items-center mb-2">
                                    <h2 className="text-xs md:text-sm font-bold">{isEditMode ? 'Edit Corporate Group' : 'New Corporate Group'}</h2>
                                    <button
                            ...
                            <form onSubmit={handleModalSubmit}>
                                <div className="mb-4">
                                    <label className="block text-[10px] md:text-xs font-medium mb-2">Group Name *</label>
                                    <input
                                        type="text"
